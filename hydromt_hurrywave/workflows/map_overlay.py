...
def make_mask_dataframe(
    ...
    Drops cells with ``mask <= 0`` and reprojects the remaining cell
    centres from ``source_crs`` to EPSG:3857. Dateline wrapping is
    applied when the source CRS is geographic.
...
def make_mask_overlay(
    ...
    colors : dict, optional
        Mapping of mask value → colour, e.g. ``{1: "yellow", 2: "red"}``.
    px : int, optional
    ...
    try:
        ...
        for mask_val, color in colors.items():
            ...
            if len(df_sub) > 0:
                ...
                # small boolean image (independent of the number of points),
                # so a scipy binary dilation is equivalent and instant.
                images.append(
                    tf.shade(
                        _dilate_bool_agg(cvs.points(df_sub, "x", "y", ds.any()), px),
                        cmap=color,
                    )
                )
