...
def merge_multi_dataarrays_on_mesh(
    ...
    for i in range(0, len(da_list)):
        merge_method = da_list[i].get("merge_method", "first")
        logger.debug(f"Processing dataset {i} with merge_method={merge_method}")
        if merge_method == "first" and not uda_out.isnull().any():
            logger.debug("Skipping dataset due to 'first' merge method and no NaNs.")
            continue
        ...
        # close the dataset and collect garbage
        da.close()
        gc.collect()
